...
const ProductDetailPage: React.FC = () => {
    ...
    const getMPProduct = (_product: Product): mParticle.Product => {
        const { label, id, price, brand, category } = _product;
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (quantity && product) {
            // console.log(`Add to Cart -- Style: ${style} | Silhouette: ${silhouette}`);
            addToCart(product, parseInt(quantity, 10), { color, size, style, silhouette });
            ...
            console.log(`Cart Price: ${mParticleProduct.Price}`)
    ...
    useEffect(() => {
        ...
        if (productId) {
            ...
            if (selectedProduct) {
                ...
                if (selectedProduct.variants?.style) {setStyle(selectedProduct.variants?.style[0]);}
                if (selectedProduct.variants?.silhouette) {setSilhouette(selectedProduct.variants?.silhouette[0]);}
            }
        }
    }, []);
        ...
        product && (
            <Page>
                ...
                <Snackbar
                    ...
                    action={
                        <Button component={Link} to='/cart' sx={{color: 'black'}}>
                            Go to Cart
                        </Button>
                    }
